...
void MapManager::LoadMap(ifstream &p_fileToRead) {
	...
	if (p_fileToRead.is_open())
	{
		...
		while (getline(p_fileToRead, line))
		{
			...
			if (i > 9)tempMap.push_back(temp);
			i++;
		}
		p_fileToRead.close();

		Map newMap(name, tempMap, id, canBattle, v_tempCoords, v_tempNPCs, v_tempChests, v_tempEnemies, defaultCoords[0], defaultCoords[1], tempSpaceColor, tempMusic);
